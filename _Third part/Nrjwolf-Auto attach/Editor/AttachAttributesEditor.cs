/*
 *  It's a tool that automatize the process of getting the reference of a component in awake/start
 *      by marking field with an attribute. It seems to hammering just when the field is null and just
 *      OnGUI messages.
 * 
...
namespace NTools
{
 ...
 {
  private Color m_GUIColorDefault = new(.6f, .6f, .6f, 1);
  private Color m_GUIColorNull = new(1f, .5f, .5f, 1);
   ...
   for (int i = 0; i < assemblies.Length; i++)
   {
    ...
    for (int n = 0; n < types.Length; n++)
    {
     if (typeof(Object).IsAssignableFrom(types[n]) && aClassName == types[n].Name)
      return Object.FindObjectOfType(types[n]);
    }
   }
   return new Object();
  }
